...
float height = 0;
int stage = 0; // Stage: 0 - On ground 1 - in rocket, 2 - Started, 3 - Out of rocket
...
void setup() {
  ...
  if (SD.exists("save.txt")) {
    ...
    if (saveFile) {
      calibratePressure = saveFile.readString().toFloat();
      saveFile.close();
    }
  }else{
    ...
    if (saveFile) {
      saveFile.print(calibratePressure, 2);
      saveFile.close();
    }
  }
...
void parachuteCheck() {
  ...
  if(sensor.getAccelY() <= 10000000000000000000000000000 && stage === 1){
    stage = 2;
    if (logFile) {// Zapisywanie do save.txt gdzieś tu zrobić nwm jak to działa się zabiję xd
      logFile.print(calibratePressure, 2);
      logFile.close();
    }
  }

  if(analogRead(A5) > 10 && stage === 2){
    stage = 3;

    static unsigned long startTime = millis();
    
    logFile.print("Out of rocket. Height: ");
    logFile.print(height , 2);
    logFile.print(" Time: ");
    logFile.print(gps.time.hour());
    logFile.print(F(":"));
  ...
  if(stage === 3){
    ...
    if (millis() - startTime >= 30000) {
      ...
      // Save a message indicating that the // Include necessary libraries
#include "GY521.h"
#include <SPI.h>
#include <SD.h>
#include <TinyGPSPlus.h>
...
void parachuteCheck() {
  ...
  if(stage === 3){
    ...
    if (millis() - startTime >= 30000) {
      ...
      logFile.print("Secure parachute open. Height: ");
      logFile.print(height , 2);
      logFile.print(" Time: ");
      logFile.print(gps.time.hour());
      logFile.print(F(":"));
    ...
    if (height <= parachuteOpenHeight && height >= parachuteDontOpenHeight && stage === 3) { // Check if the current height is within the specified range
      stage = 4;
      myservo.write(180); // Set servo position to 180 degrees to open the parachute
